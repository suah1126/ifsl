...
#             box_query_img.rectangle(object_box['bbox'], outline=(0, 255, 0), width=3)
...
#         img_pil.save('./example/s_'+query_name+'.jpg', 'JPEG')
    ...
    def get_query_box(self, org_imsize, query_imsize, query_cbox, rename_class):
        ...
        for box in query_cbox:
            if self.split == 'trn':
                box[1], box[2], box[3], box[4] = int(box[1]*x_ratio), int(box[2]*y_ratio), int(box[3]*x_ratio), int(box[4]*y_ratio)
            box[0] = rename_class(box[0])
        return query_cbox
    ...
    def get_support_boxes(self, support_box, org_imsize, support_imsize, class_id, rename_class):
        x_ratio = support_imsize[0] / org_imsize[0]
        y_ratio = support_imsize[1] / org_imsize[1]
    ...
    def read_box(self, img_name):
        ...
        boxes = []
        for obj in ET.parse(os.path.join(self.bbox_path, img_name) + '.xml').getroot().findall('object'):
