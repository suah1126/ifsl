...
class DatasetPASCAL(Dataset):
    ...
    """
    def __init__(self, datapath, fold, transform, split, way, shot, task, finetune=False):
        ...
        self.shot = shot
        self.task = task
        self.allfold = True if finetune or self.task == 'det' and self.split == 'val' else False
        self.way = 20 if self.allfold else way
    ...
    def __len__(self):
        return len(self.img_metadata) if self.split == 'trn' else 100
    ...
    def build_class_ids(self):
        ...
        if self.allfold:
            class_ids_trn.extend(class_ids_val)
    ...
    def build_img_metadata(self):
        ...
        elif self.split == 'trn':  # For training, read image-metadata of "the other" folds
            for fold_id in range(self.nfolds):
