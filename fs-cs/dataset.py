...
from common.detection import read_truths_args, read_truths, is_dict
from image import *
...
def build_dataset(dataopt):
    ...
    if cfg.repeat == 1:
        ...
        if 'dynamic' not in dataopt or int(dataopt['dynamic']) == 0:
            ...
            metalist, metacnt = load_metadict(dataopt['meta'], cfg.repeat)                  # novel class와 box 몇개가 들어있는지 읽어옴.
            return build_fewset(dataopt['train'], metalist, metacnt, cfg.shot*cfg.repeat)   # novel class+base class
...
def build_fewset(imglist, metalist, metacnt, shot, replace=True):
    ...
    while min(metacnt.values()) < shot:
        ...
        overflow = False
        for ci in set(bcls):
            if metacnt[cfg.classes[ci]] + bcls.count(ci) > shot:        #base class의 box 개수 + 현재 이미지에서 추가될 box 개수 > shot
                overflow = True
                break
        if overflow:
            names.remove(imgpath)
        ...
        for ci in set(bcls):
            metacnt[cfg.classes[ci]] += bcls.count(ci)                  #base class box 개수 업데이트
    ...
    return metalist                                                     #base+novel list
    ...
    def is_valid(imgpath):
        ...
        if os.path.getsize(labpath):
            ...
            if bs is not None:
                bs = np.reshape(bs, (-1, 5))
                clsset = set(bs[:,0].astype(np.int).tolist())
                if not clsset.isdisjoint(set(cfg.base_ids)):            #base class 포함하는지 확인
                    return True
        return False
...
class MetaDataset(Dataset):
            ...
            for i, clsname in enumerate(self.classes):
                with open(metafiles[clsname], 'r') as imgf:
                    ...
                    if ensemble:
                        ...
                        inds = np.random.choice(range(len(lines)), nbatch).tolist()             #voclist/%class_train.txt에서 랜덤한 이미지 선택
                        metainds[i] = list(zip([i] * nbatch, inds))                             #metainds[0] = [(0, 1), (0, 23) ...]

        self.inds = sum(metainds, []) if ensemble else sum(list(zip(*metainds)), ())            #tuple 형태: (0,1), (0,23), (1, 24), (1, 25) ...
        self.meta_cnts = [len(ls) for ls in self.metalines]
        if cfg.randmeta:                                                                        #self.inds tuple을 섞을 것인지
            self.inds = list(self.inds)
            random.shuffle(self.inds)
            self.inds = tuple(self.inds)

        self.with_ids = with_ids                                                                #batch 구성
    ...
    def get_img_mask(self, img, box, merge=True):       # xywh to xyxy
        w, h = self.mask_shape
        ...
        if meta_lab:
            for lab in meta_lab:
                ...
                if mask is None:
                    continue
                return (img, mask)
        # multiple object인 경우, box 한개만 가져옴. 
    ...
    def filter(self, inds):         #self.inds에 있는 img만 가져옴.
        newinds = []
        print('===> filtering...')
        _cnt = 0
...
if __name__ == '__main__':
    ...
    trainlist = '../datasets/2012_train.txt'
