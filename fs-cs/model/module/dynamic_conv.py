...
def dynamic_conv2d(is_first, partial=None):
    ...
    class DynamicConv2d(_ConvNd):
        ...
        def forward(self, inputs):
            '''
            qry_feat: [bsz, 1024, 13, 13]
            dynamic_weight: [bsz*way, 1024, 1, 1]
            '''
            # qry_feat, dynamic_weight = inputs
            # bsz, ch, h, w = qry_feat.size()
            # way = dynamic_weight.size(0) // bsz
            # # qry_feat = qry_feat.view(1, -1, h, w).repeat(1, way, 1, 1)
            # # dynamic_weight = dynamic_weight.view(-1, 1, 1, 1)

            # #print('dynamic:', qry_feat.shape, dynamic_weight.shape)
            # # [bsz, 1024 * way, 13, 13] --> [1, bsz * 1024 * way, 13, 13]
            # # [1024 * way, 1, 1, 1] , group = bsz / 1024 * way
            # # [bsz * way, 1024, 13, 13]

            # qry_feat = qry_feat.view(1, -1, h, w)
            # dynamic_weight = dynamic_weight.view(-1, 1, 1, 1)
            # x = F.conv2d(qry_feat, dynamic_weight, self.bias, self.stride, self.padding, self.dilation, groups= bsz * ch)
            # x = x.view(-1, ch, h, w)
            # return x
            ...
            # [bsz, 1024 * way, 13, 13] --> [1, bsz * 1024 * way, 13, 13]
            # [bsz * way, 1024, 1, 1] --> [bsz * way, 1024, 1, 1]
            #[bsz, way, ]
            # weights = weights.view(-1, 3, 5, 5)
            # print(weights.shape)
            # > torch.Size([150, 3, 5, 5])
            # # move batch dim into channels (10, 3, 24, 24)
            # x = x.view(1, -1, H, W)
            # print(x.shape)
